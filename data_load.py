...
def load_vocab():
    vocab = [line.split()[0] for line in open('data/vocab', 'r') if int(line.split('\t')[1])>=hp.min_cnt]
...
def load_train_data():
    source_sents = [line for line in open(hp.source_train, 'r')]
    target_sents = [line for line in open(hp.target_train, 'r')]
    
    X, Y, Sources, Targets = create_data(source_sents, target_sents)
    return X, Y

def load_test_data():
    source_sents = [line for line in open(hp.source_test, 'r')]
    target_sents = [line for line in open(hp.target_test, 'r')]
